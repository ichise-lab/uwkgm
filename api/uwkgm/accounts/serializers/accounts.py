...
class CustomUserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        exclude = conf.resolve('accounts.fields.full.exclude')
    ...
    def _set_password(self, user: CustomUser) -> CustomUser:
        ...
        if self.data_password is not None:
            validate_password(self.data_password, user=user)
...
class CustomUserLimitedSerializer(CustomUserSerializer):
    """An additional 'CustomUser' serializer that limits account information being sent to the user since
    some information are not useful for viewing (e.g. password) and some are sensitive (e.g. 'is_superuser')
    """

    class Meta(CustomUserSerializer.Meta):
        exclude = conf.resolve('accounts.fields.limited.exclude')
        read_only_fields = conf.resolve('accounts.fields.limited.read_only')
